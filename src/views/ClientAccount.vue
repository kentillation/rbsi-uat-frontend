  ...
  <v-container>
    ...
    <v-sheet class="d-flex flex-column align-center text-center mx-auto" elevation="4" height="250" width="100%"
      ...
      <div class="w-75 mt-10">
        ...
        <v-btn prepend-icon="mdi-magnify" class="bg-teal-darken-4 ms-2" size="large"
          :disabled="!searchValidAcc || validatingAcc" @click="searchAccount" rounded>
    ...
    <v-dialog v-model="dialogAccountDetails" transition="dialog-bottom-transition" width="600px" persistent>
      <v-card>
        ...
        <v-card-text>
          <v-container>
            <v-row v-if="selectedAccount">
              <v-col cols="12">
                <p><span class="text-teal-darken-1">Outstanding Balance:</span><strong> ₱{{ formatCurrency(selectedAccount.balAmt) }}</strong></p>
                <p><span class="text-teal-darken-1">Available Balance:</span><strong> ₱{{ formatCurrency(selectedAccount.availBalAmt) }}</strong></p>
                <p><span class="text-teal-darken-1">Interest Rate: </span><strong> {{ selectedAccount.intRate }}%</strong></p>
                <p><span class="text-teal-darken-1">Interest Effective Date: </span><strong> {{ formatDate(selectedAccount.intEffDate) }}</strong></p>
                <p><span class="text-teal-darken-1">Account Status: </span><strong>{{ selectedAccount.accStatus }}</strong></p>
                <p><span class="text-teal-darken-1">Status Date: </span><strong> {{ formatDate(selectedAccount.accStatusDate) }}</strong></p>
                <p><span class="text-teal-darken-1">Currency Type: </span><strong> {{ selectedAccount.ccyType }}</strong></p>
                <p><span class="text-teal-darken-1">Open Date: </span><strong> {{ formatDate(selectedAccount.openDate) }}</strong></p>
              </v-col>
            </v-row>
            <v-row v-else>
        ...
        <v-card-actions class="mx-4 my-4">
          <v-btn class="bg-teal-darken-4 px-3" prepend-icon="mdi-history" @click="showDialogTrnsctnHstry"
    ...
    <v-dialog v-model="dialogSearchTransactionHistory" transition="dialog-bottom-transition" width="600px" persistent>
      <v-card>
        <v-card-title>
          <span class="headline">Search Transaction History</span>
        </v-card-title>
        <v-card-text>
          <v-container>
            <v-row>
        ...
        <v-card-actions class="mx-4 my-4">
          <v-spacer></v-spacer>
          <v-btn class="bg-red-darken-4 px-3" prepend-icon="mdi-close-circle-outline"
            @click="dialogSearchTransactionHistory = false" rounded>
            Close
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="dialogTransactionHistoryTable" transition="dialog-bottom-transition" width="1000px" persistent>
      <v-card>
        <v-card-title>
...
export default {
  ...
  watch: {
    ...
    search_item_ACC(newVal) {
      ...
      this.search_item_ACC = this.formatAcc(newVal);
    }
  },
  methods: {
    dialogOpenCID() {
    ...
    showDialogTrnsctnHstry() {
      ...
      this.dialogSearchTransactionHistory = true;
    },
    async searchCID() {
    ...
    async searchAccount() {
      ...
      try {
        const account = this.search_item_ACC.replace(/-/g, '');
        if (account.length !== 8) {
          this.$refs.snackbarRef.showSnackbar("Invalid account number", "error");
          return;
        }
